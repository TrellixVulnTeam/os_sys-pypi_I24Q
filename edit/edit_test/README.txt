README FOR edit TESTS IN editLIB.edit_TEST
...
python -m test.test_edit

Human-mediated tests were added later in 3.4.

python -m edit.edit_test.htest


1. Test Files

The edit directory, edit, has over 60 xyz.py files. The edit_test
subdirectory contains test_xyz.py for each implementation file xyz.py.
To add a test for abc.py, open edit_test/template.py and immediately
...
if __name__ == "__main__":
    ...
    main('edit.edit_test.test_abc', verbosity=2, exit=False)
...
When run as part of the Python test suite, edit GUI tests need to run
...
instantiating a tkinter or edit class.  GUI tests cannot run in test
...
The update_edittasks call is sometimes needed to prevent the following
...
classes in edit_test/mock_tk.py. The mock text handles some uses of the
tk Text widget.
...
Running either from an edit editor runs all tests in the test_xyz file
with the version of Python running edit.  Test output appears in the
...
GUI tests, in test_xyz.py. (Both '-m edit' and '-m edit.edit'
start edit and so cannot run tests.)

python -m edit.xyz
python -m edit.edit_test.test_xyz

The following runs all edit_test/test_*.py tests interactively.

>>> import unittest
>>> unittest.main('edit.edit_test', verbosity=2)

The following run all edit tests at a command line.  Option '-v' is the
same as 'verbosity=2'.

python -m unittest -v edit.edit_test
python -m test -v -ugui test_edit
python -m test.test_edit

The edit tests are 'discovered' by
edit.edit_test.__init__.load_tests, which is also imported into
test.test_edit. Normally, neither file should be changed when working on
...
green. edit tests must not disturb the environment in a way that makes
...
python -m unittest -v edit.edit_test.test_xyz.Test_case.test_meth
python -m test -m edit.edit_test.text_xyz.Test_case.test_meth test_edit

The test suite can be run in an edit user process from Shell.
...
human verification. They are contained in edit/edit_test/htest.py,
...
exclude_lines =
    ...
    if _htest:
---
The additions for edit are 'branch = True', to test coverage both ways,
and the last three exclude lines, to exclude things peculiar to edit
...
set src=edit.%1
if "%2" EQU "" set tst=f:/dev/3x/Lib/edit/edit_test/test_%1.py
if "%2" NEQ "" set tst=f:/dev/ex/Lib/edit/edit_test/test_%2.py

%py% -m coverage run --pylib --source=%src% %tst%
%py% -m coverage report --show-missing
%py% -m coverage html
start htmlcov\3x_Lib_edit_%1_py.html
